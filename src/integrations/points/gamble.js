...
function gamble(user, amountInput) {
  ...
  if (now < readyAt) {
    return `@${name} gamble is on cooldown for ${Math.ceil((readyAt - now) / 1000)}s.`
  }

  const balance = points.getBalance(user)
  const bet = points.parseAmount(amountInput, balance)

  if (bet === null) return `@${name} "${amountInput}" isn't a valid amount. Try a number, a percentage, or "all".`
  if (bet < cfg.minBet) return `@${name} the minimum bet is ${points.format(cfg.minBet)} ${currency}.`
  if (bet > balance) return `@${name} you only have ${points.format(balance)} ${currency}.`
  ...
  if (won) {
    ...
    return `@${name} gambled ${points.format(bet)} ${currency} and WON! +${points.format(bet)} → ${points.format(total)} ${currency} PogChamp`
  }

  const total = points.removePoints(user, bet)
  return `@${name} gambled ${points.format(bet)} ${currency} and lost it all. → ${points.format(total)} ${currency} Sadge`
}
